...
class WorkoutExerciseActivity : AppCompatActivity() {
    ...
    // 🎯 [추가] 폴더별 저장을 위해 전달받은 폴더 정보를 담을 변수
    ...
    private val finishResultLauncher = registerForActivityResult(
        ...
        if (result.resultCode == Activity.RESULT_OK) {
            // WorkoutFinishActivity가 성공적으로 저장하고 돌아왔을 때
            val intentData = result.data
            setResult(Activity.RESULT_OK, intentData)

            // 🎯 타이머를 아예 초기화하고 액티비티를 닫습니다.
            TimerManager.pauseTimer()
            // TimerManager.resetTimer() // 리셋 함수가 있다면 호출

            finish() // 이 화면을 닫아야 타이머 관찰(Observe)도 멈춥니다.
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 🎯 [추가] 이전 화면(Fragment 또는 Activity)에서 보낸 폴더 정보 수신
    ...
    private fun moveToFinishActivity() {
        // 1. 현재 화면의 UI 업데이트부터 즉시 차단 (이게 저번 프로젝트의 핵심 팁!)
        ...
        // 2. 🎯 [가장 중요] 타이머를 '일시정지'가 아니라 '완전 초기화' 합니다.
        // 그래야 seconds가 0이 되고, 메인 화면의 플로팅 바가 isTimerActive() -> false가 되어 사라집니다.
        TimerManager.stopAndResetTimer()

        val intent = Intent(this, WorkoutFinishActivity::class.java).apply {
            putExtra("FOLDER_ID", folderId)
            putExtra("TOTAL_TIME", totalTimerTime) // 이미 위에서 변수에 담았으니 0이 되어도 상관없음
