...
class WorkoutActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 상단 헤더의 날짜와 이모티콘 뷰 (condition_emoticon)
        ...
        // 🚨 [동기화 연동] 프래그먼트에서 수송해준 고유 해시 이모티콘 ID를 수신합니다.
        // 데이터 누락을 식별할 수 있도록 기본값은 -1로 설정합니다.
        val emojiResId = intent.getIntExtra("EMOJI_RES_ID", -1)
        val startTab = intent.getIntExtra("SELECT_TAB", 0)

        // Logcat에서 'JaehoonTest' 태그로 검색
        ...
        if (emojiResId != -1) {
            // 🚨 프래그먼트에서 연산된 고유 랜덤 색상 이모티콘을 텍스트뷰 옆에 정확히 꽂아줍니다!
            ivHeaderEmoji.setImageResource(emojiResId)
            ivHeaderEmoji.visibility = View.VISIBLE
            Log.d("JaehoonTest", "전달받은 고유 이모티콘 적용 성공")
        } else {
            // 데이터를 못 받은 경우 (기본 이미지 세팅 및 경고 로그)
            Log.e("JaehoonTest", "⚠️ EMOJI_RES_ID가 전달되지 않았습니다. 기본 이모티콘을 출력합니다.")
