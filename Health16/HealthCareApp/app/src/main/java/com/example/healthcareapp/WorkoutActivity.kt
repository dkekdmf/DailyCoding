...
class WorkoutActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        TabLayoutMediator(tabLayout, viewPager) { tab, position ->
            ...
            customView.setTextColor(Color.parseColor(if (position == 0) "#2E3D4F" else "#8896A8"))
            tab.customView = customView
        }.attach()

        tabLayout.addOnTabSelectedListener(object : TabLayout.OnTabSelectedListener {
            override fun onTabSelected(tab: TabLayout.Tab?) {
                (tab?.customView as? TextView)?.setTextColor(Color.parseColor("#2E3D4F"))
            ...
            override fun onTabUnselected(tab: TabLayout.Tab?) {
                (tab?.customView as? TextView)?.setTextColor(Color.parseColor("#8896A8"))
            }
...
//    private fun getVisibleExerciseFragment() = supportFragmentManager.fragments.find { it is com.example.healthcareapp.fragment.WorkoutRecordFragment } as? com.example.healthcareapp.fragment.WorkoutRecordFragment
    ...
    private fun showCompletionDialog(btnFrame: FrameLayout, btnTextView: TextView) {
        ...
        dialogView.findViewById<View>(R.id.btn_yes).setOnClickListener {
            isEditMode = false // 1. 액티비티 모드 변경

            // 🎯 [핵심 추가] 프래그먼트에게 편집 모드 종료(false)를 전달하세요!
            ...
            // 3. UI 갱신 (데이터 다시 로드)
            getVisibleExerciseFragment()?.onResume()
            getVisibleConditionFragment()?.refreshData()
