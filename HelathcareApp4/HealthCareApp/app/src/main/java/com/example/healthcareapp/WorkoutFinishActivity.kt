...
class WorkoutFinishActivity : AppCompatActivity() {
    ...
    private fun initClickListeners() {
        ...
        binding.btnFinishWorkout.setOnClickListener {
            val totalTime = binding.tvTotalTime.text.toString()
            ...
            DiaryPrefsManager.saveJournal(this, folderId, newJournal)

            // 3. 결과 전달 후 종료
            setResult(Activity.RESULT_OK, Intent().apply { putExtra("EMOJI_RES_ID", selectedEmojiResId) })
            finish()
        }
    }

    /**
     * 🎯 [추가] DiaryPrefsManager를 사용하여 로컬에 일지 저장
     */
    private fun saveToDiary(memo: String, totalTime: String, emojiId: Int) {
        // 캘린더와 리스트에서 동일하게 사용할 날짜 포맷 (yyyy-MM-dd)
        val fullDate = SimpleDateFormat("yyyy-MM-dd HH:mm", Locale.getDefault()).format(Date())
        ...
        if (folderId != -1L) {
            DiaryPrefsManager.saveJournal(this, folderId, newJournal)
            android.util.Log.d("JaehoonLog", "💾 저장 완료: 폴더 $folderId, 시간 $totalTime")
        } else {
            android.util.Log.e("JaehoonLog", "❌ 폴더 ID가 없어 저장에 실패했습니다.")
        }
    }
