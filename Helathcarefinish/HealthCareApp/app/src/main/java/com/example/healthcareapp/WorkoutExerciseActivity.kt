...
class WorkoutExerciseActivity : AppCompatActivity() {
    ...
    // WorkoutExerciseActivity.kt 클래스 내부에 추가
    override fun onNewIntent(intent: Intent) {
        super.onNewIntent(intent)
        setIntent(intent) // 🚀 이게 없으면 getIntent()는 계속 예전 인텐트만 봅니다.

        // 🎯 중요: 돌아올 때 인텐트에서 folderId를 다시 추출해서 업데이트하세요!
        val updatedFolderId = intent.getLongExtra("FOLDER_ID", -1L)
        if (updatedFolderId != -1L) {
            this.folderId = updatedFolderId
            Log.d("JaehoonLog", "🚀 onNewIntent: 폴더 ID 갱신됨 -> $folderId")
        }
    ...
    private fun syncTimer() {
        ...
        binding.btnPause.setImageResource(if (TimerManager.isRunning()) R.drawable.puase3x else R.drawable.play3x)
    }
    ...
    private fun moveToFinishActivity() {
        ...
        Log.d("JaehoonFinalCheck", "현재 folderId: $folderId")

        // 2. 여기서 리스트 사이즈 확인
        Log.d("JaehoonFinalCheck", "개인운동 리스트: ${personalWorkoutList.size}, PT 리스트: ${ptWorkoutList.size}")
    ...
    private fun refreshWorkoutListUI() {
        val currentList = if (isPtMode) ptWorkoutList else personalWorkoutList
        workoutAdapter = RecordWorkoutAdapter(currentList)
        binding.rvWorkoutList.adapter = workoutAdapter
        workoutAdapter.notifyDataSetChanged() // 화면에 다시 그리도록 강제함
        Log.d("JaehoonLog", "UI 갱신 완료 - 리스트 사이즈: ${currentList.size}")
    }
